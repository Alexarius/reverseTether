...
configs/
  prompts/
    suite.json           # Versioned prompt suite for benchmark runs
  conditions/
    matrix.json          # Matrix run condition definitions
  server_metadata_schema.json  # Schema for server metadata validation
```

## Prompt Suite (`prompts/suite.json`)

The prompt suite is a versioned JSON file containing all prompts used in benchmark runs. This ensures reproducibility and prevents prompt drift.
...
1. **Prompt IDs** use the format `<tier>_v<version>` (e.g., `short_v1`, `soak_v2`)
...
1. Increment the version in the `id` field (e.g., `short_v1` → `short_v2`)
...
# Load the suite
suite = load_prompt_suite(Path("configs/prompts/suite.json"))

# Get prompt text and ID for a tier
prompt_text, prompt_id = get_prompt_for_tier(suite, "short")
# prompt_id will be "short_v1" - logged in raw_metrics.jsonl
```
