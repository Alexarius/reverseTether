...
class TestAggregateFiltering(unittest.TestCase):
    ...
    def test_default_aggregation_excludes_unknown_and_missing_cache_policy(self):
        """Default aggregation should reject unverifiable final cache policies."""
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "results"
            output_dir = root / "summaries"
            ...
            self.assertEqual(exit_code, 0)
            self.assertIn(
                "Final evidence strict filter dropped 2 records.",
                "\n".join(logs.output),
            )
    ...
    def test_include_smoke_keeps_development_records(self):
        """The include-smoke flag should opt in to unfiltered aggregation."""
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "results"
            output_dir = root / "summaries"
            ...
            run_output_dir = next(path for path in output_dir.iterdir() if path.is_dir())
            raw_export = pd.read_csv(run_output_dir / "raw_export.csv")
            self.assertEqual(raw_export["run_id"].tolist(), ["smoke-run", "final-run"])
