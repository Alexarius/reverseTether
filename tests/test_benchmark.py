...
class TestBenchmarkLogging(unittest.TestCase):
    ...
    def test_run_benchmark_writes_metadata_and_populates_token_timestamps(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(metadata["prompt_suite_type"], "smoke")
            ...
            self.assertEqual(metadata["prompt_fixture_sha256"], expected_hash)
            ...
            self.assertEqual(raw_record["prompt_suite_type"], "smoke")
    ...
    def test_matrix_jsonl_all_records_have_prompt_metadata(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            for line in lines:
                ...
                self.assertIn("prompt_suite_type", record)
                ...
                self.assertEqual(record["suite_type"], "smoke")
                self.assertEqual(record["prompt_suite_type"], "smoke")
                self.assertEqual(record["prompt_suite_id"], "smoke_suite")
                self.assertEqual(record["prompt_suite_version"], "1.0.0")
            ...
            expected_short_hash = hashlib.sha256(b"Test prompt").hexdigest()
            expected_soak_hash = hashlib.sha256(b"Soak prompt").hexdigest()
            self.assertEqual(
                [
                    (
                        record["regime"],
...
class TestMatrixRunner(unittest.TestCase):
    ...
    def test_run_matrix_handles_failed_runs(self):
        ...
        self.assertEqual(failed_record["prompt_suite_type"], "smoke")
